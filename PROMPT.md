...
8. Push this iteration's commit with `git push` (use `git push -u origin <branch>` on a new branch).
   The pre-push hook runs the full verify and your work is saved remotely. Push every iteration so
   commits never pile up locally.
