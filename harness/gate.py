...
from harness.preferences import MAX_FUNCTIONS_PER_FILE, preferences_violations
...
FULL_CHECKS: Checks = (
    ...
    ("security", ("uv", "run", "semgrep", "scan", "--config", "auto", "--error", "--quiet")),
    ("tests", ("uv", "run", "pytest")),
)
...
def staged_preferences_violations(repo: Path) -> list[str]:
    """Run structural style checks on the staged content of changed Python files."""
    ...
    for path in out.splitlines():
        ...
        limit = 0 if is_test_file else MAX_FUNCTIONS_PER_FILE
        problems.extend(preferences_violations(path, run_git(repo, ["show", f":{path}"]), limit))
    return problems
